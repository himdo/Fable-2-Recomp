...
├── src/                          <- host application code (main.cpp, fable_2_app.h)
│   ├── main.cpp
│   └── fable_2_app.h             <- OnConfigurePaths() implements the content-root search
...
- Toolchain: the `win-amd64-debug` preset (Ninja + clang). The SDK's public headers use clang builtins (`__builtin_bswap32`, `__VA_OPT__`), so plain MSVC `cl` cannot compile the generated code — a `win-msvc-debug` preset exists but only works for targets that don't compile `generated/`.
