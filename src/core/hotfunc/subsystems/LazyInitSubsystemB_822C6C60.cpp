// Hand-tuned override of recompiled LazyInitSubsystemB_822C6C60
// (recompiler output: fable_2_recomp.215.cpp; generated transforms nv=1 mfmsr=1 glock=0 gt=0).
...
// of __imp__LazyInitSubsystemB_822C6C60; fable_2_register.cpp registers the ALIAS in the
// indirect-dispatch table. This strong definition intercepts both direct bl
// calls and bctrl dispatch. __imp__LazyInitSubsystemB_822C6C60 remains the original.
//
// Guest logic - one-time subsystem initializer that returns the subsystem
// object pointer (0x834C3260) in r3 on every path:
//
//   u32* LazyInitSubsystemB()
//   {
//     u32& flag = *(u32*)0x834C3508;          // bit 0 = "already initialized"
//     if ((flag & 1) == 0) {
//       flag |= 1;                             // claim the slot (not atomic)
//       sub_824F73A8(0x834C3260);              // construct the subsystem
//       ProcessAndAllocateAndProcess2_82CA3700(0x832A2C50);  // register/allocate
//     }
//     return 0x834C3260;
//   }
...
extern "C" void sub_824F73A8(PPCContext& ctx, uint8_t* base);
...
extern "C" void LazyInitSubsystemB_822C6C60(PPCContext& __restrict ctx, uint8_t* base) {
  REX_FUNC_PROLOGUE();
  ...
  // One-time init guard on flag bit 0 at 0x834C3508.
  const u32 kFlag = 0x834C3508;               // r10.u32 + 13576
  const u32 flag = gload32(kFlag, base);       // lwz r11,13576(r10)
  ctx.cr6.compare<u32>(flag & 1, 0, ctx.xer);  // clrlwi r9,r11,31; cmplwi cr6,r9,0
  if ((flag & 1) == 0) {             // bne cr6 -> already initialized, skip
    gstore32(kFlag, base, flag | 1); // ori r11,r11,1; stw r11,13576(r10)

    const i64 subsystem = kBase + 12896;       // addi r31,r9,12896 -> 0x834C3260 (sign-extended)
    ctx.r31.s64 = subsystem;          // (callee-saved; the construct call sees it)
    ctx.r3.s64 = subsystem;           // mr r3,r31
    ctx.lr = 0x822C6C9C;              // bl 0x824f73a8
    sub_824F73A8(ctx, base);
    // lis r8,-31958 (0x832A0000, sign-extended); addi r3,r8,11344 -> 0x832A2C50
    ctx.r3.s64 = static_cast<i64>(static_cast<i32>(0x832A0000)) + 11344;
    ctx.lr = 0x822C6CA8;              // bl 0x82ca3700
    ProcessAndAllocateAndProcess2_82CA3700(ctx, base);
  }

  // --- Return value on every path: r3 = 0x834C3260 (sign-extended) ---
  ctx.r3.s64 = kBase + 12896;         // (init path: mr r3,r31; else path: addi r3,r11,12896)
